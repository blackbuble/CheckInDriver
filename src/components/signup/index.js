...
import { View, TextInput, StyleSheet, TouchableOpacity, TouchableHighlight, Image, ScrollView, BackHandler} from 'react-native'
...
class Signup extends Component {
 ...
 renderButtton() {
  ...
  onSubmit = () => {
	  this.setState({loading: true}); 
	  const { email, password, passwordOne,fullname,mobile_phone,photo,reference,photoURI,balance,photoIDCard,photoSIM,photoSTNK,car_type,car_number,working_area,category,bank,isActive, createdAt } = this.state;
	  //console.log(this.state)
	  ...
	  .then((authData) => {
		  ...
		  //this.userProfile();
		  const user = { email, fullname, mobile_phone, reference,photoURI, balance, photoIDCard,photoSIM,photoSTNK,car_type,car_number,working_area,category,bank, isActive}
		  console.log(user)
		  ...
		  .then( (data) => {
			  ...
			  const inbox = { title:'Selamat Bergabung', body:'Hai..selamat bergabung dengan CheckIn. Untuk mendapatkan order terbaru, selalu cek aplikasi CheckIn', key: userID, createdAt: new Date()}
